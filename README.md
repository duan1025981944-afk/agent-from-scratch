...
- 跨会话记忆：自己整理长期记忆；记错了可以 `/correct` 纠正、`/why` 查出处、`/memory-undo` 退回上一版
...
/correct 正确的说法   记忆记错了，纠正一条（记一笔 correction 并立刻整理）
/why 关键词           查某句话是哪来的：在流水账里找出处和所属会话
...
│   ├── SKILLS.md        常驻层：技能清单的引导语（清单本身由程序接在后面）
│   └── prompts/
...
│       ├── load_skill.py 按名字读技能正文（收名字不收路径，防线是查表）
│       └── *.py         具体工具，一个文件一个
│
...
三十四条「为什么放在这里」、与 nanobot 的差异、负对照的测试写法）。
