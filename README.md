...
```
main.py                  命令行入口，决定展示什么
│
├── config/loader.py     配置加载，支持 ${ENV_VAR} 占位符
├── providers/           模型服务抽象（Provider 基类 + 工厂）
│
├── agent/
│   ├── runner.py        主循环：请求模型 → 判断要不要用工具 → 执行 → 下一圈
│   ├── payload.py       发送前加工：把「存档」变成「这一次要发的东西」
│   └── tools/
...
│
└── session/
    ├── manager.py       会话生命周期：新建 / 恢复 / 列出
    └── messages.py      jsonl 底层读写
```
...
用户输入
   ↓
messages.append(user)              记下写盘边界 boundary
   ↓
┌─ runner 循环 ─────────────────────────────┐
│  build_payload(messages) ──→ 模型          │   ← 截断在这里发生
│  messages.append(assistant)                │
│  要用工具？ ── 是 ──→ 执行 ──→ append(tool) │   ← 存的是全文
│              └─ 否 ──→ 退出循环             │
└───────────────────────────────────────────┘
   ↓
session.save_turn(messages[boundary:])       整轮一次性落盘
```
...
| 点 | nanobot | 本项目 | 理由 |
|---|---|---|---|
| 截断时机 | `max_tool_result_chars` 在执行时生效 | 抽出独立的 `build_payload()`，仅在发送前生效 | 存档保真；调整阈值后历史立即按新值生效 |
| 压缩与存档 | 摘要会替换 history 中的原文 | 存档只增不改，压缩只作用于发送前 | 压缩有损，不应影响原始记录 |
| 会话文件头 | —— | jsonl 首行写 `_meta`（模型、工作区、创建时间） | `--list` 只需读一行；恢复时可检测工作区变更 |

（请按你实际核对过的源码填写，没确认过的不要写。）
这些选择都有对应的实测，不是拍脑袋定的。
